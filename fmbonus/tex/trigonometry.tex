...
\begin{enumerate}
 ...
 \item
  ...
  for small \(x\)''. This has many applications in engineering and physics. This
  fact is \emph{only} true if you're working in radians, which gives you some
  idea as to why radians are so immensely natural and useful.
 ...
 \item
  Evaluate the sum
  \begin{equation*}
   \sum_{k = 1}^{90} \sin^2(\degrees{k})
  \end{equation*}
  (ie, \(\sin(\degrees{k}) = \sin \frac{\pi k}{180}\)). By ``evaluate'', I mean
  write down its value without the use of trig functions.
\end{enumerate}
